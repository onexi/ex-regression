...
exercise.run =  function (){
    ...
    for (var i=0; i<length; i++){
        x[i] = getRandomNumber(); //comment out to test
        y[i] = getRandomNumber(); //comment out to test

        //x[i] = i; // just to test solution
        //y[i] = i + 1; // just to test solution
        //x[i] = i; // just to test solution
        //y[i] = 2*i + 0.5; // just to test solution
    }

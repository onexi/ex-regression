var canvas = document.getElementById('canvas'); //canvas is defined on the html run file
var ctx = canvas.getContext("2d"); //webgl can get 3d, this is a 2d drawing tool
var width = 800; //pixel dimensions
var height = 600; //pixel dimensions
...
function drawPoint(x, y, color) {
    ctx.fillStyle = color;
    y = Math.abs(y - height); //since 0,0 is at top left, this is flipping the axis
    ctx.beginPath();
    ctx.arc(x, y, 5, 0, Math.PI*2, true); //x,y location, 5 = radius, 0=start, pi*2 is the arc end
    ctx.fill();
}
